...
Dictionary<string, Dictionary<string, int>> recipeDict = new Dictionary<string, Dictionary<string, int>>();
...
public Program()
{
    ...
    for (var i = 1; i < splitted.Length; i++)
    {
        ...
        for (var j = 1; j < blocks.Length; j++)
        {
            ...
            try
            {
                recipeDict.Add(blockName, new Dictionary<string, int>());
            }
            catch (Exception)
            {
            ...
            foreach (var component in components)
            {
                ...
                recipeDict[blockName].Add(compName, amount);
            }
        }
    }
}
...
public Dictionary<string, int> GetComponents(string blockName)
{
    return recipeDict[blockName];
}


public void AddComponents(Dictionary<string, int> addTo, Dictionary<string, int> recipes, int times = 1)
{
    foreach (KeyValuePair<string, int> component in recipes)
    {
        if (component.Key.Contains("EngineerPlushie")) // Plushie components can't be assembled
            continue;
...
private List<KeyValuePair<string, int>> GetNeedComponents(IMyProjector projector)
{
    ...
    // foreach(KeyValuePair<string, int>kvp in totalComponents)
    // {
    //     Echo($"Name {kvp.Key}, cnt {kvp.Value}");
    // }
