...
namespace Sandbox.Engine.Voxels
{
	internal sealed class MyCsgTorus : MyCsgShapeBase
	{
		...
		private float SignedDistanceInternal(float lodVoxelSize, IMyModule macroModulator, IMyModule detailModulator, ref Vector3 localPosition, ref float signedDistance)
		{
			...
			{
				float num = 0.5f * m_deviationFrequency;
				Vector3 vector = localPosition * num;
				float num2 = (float)macroModulator.GetValue(vector.X, vector.Y, vector.Z);
				signedDistance -= num2 * m_secondaryHalfDeviation;
			}
			if (m_enableModulation && 0f - m_detailSize < signedDistance && signedDistance < m_detailSize)
			{
				float num3 = 0.5f * m_detailFrequency;
		...
		{
			MatrixD matrixD = MatrixD.CreateTranslation(m_translation) * worldTranslation;
			float num = (m_primaryRadius + m_secondaryRadius) * 2f;
			float num2 = m_secondaryRadius * 2f;
			MatrixD matrixD2 = MatrixD.CreateScale(num, num2, num);
			MatrixD.CreateFromQuaternion(ref m_invRotation, out var result);
			MatrixD.Transpose(ref result, out result);
			MyRenderProxy.DebugDrawCylinder(matrixD2 * result * matrixD, color.ToVector3(), 0.5f, depthRead: true, smooth: false);
		}
