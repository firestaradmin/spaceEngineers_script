...
namespace VRageMath
{
	...
	public class MyDynamicAABBTree
	{
		...
		public ReadOnlySpan<DynamicTreeNode> Nodes => new ReadOnlySpan<DynamicTreeNode>(m_nodes, 0, m_nodeCapacity);
		...
		private void InsertLeaf(int leaf, bool rebalance)
		{
			...
			while (!m_nodes[num].IsLeaf())
			{
				...
				if (rebalance)
				{
					...
					{
						BoundingBox.CreateMerged(ref original, ref m_nodes[child].Aabb, out var result);
						num4 = result.Perimeter + num3;
					}
					else
					{
						BoundingBox.CreateMerged(ref original, ref m_nodes[child].Aabb, out var result2);
					...
					{
						BoundingBox.CreateMerged(ref original, ref m_nodes[child2].Aabb, out var result3);
						num5 = result3.Perimeter + num3;
					}
					else
					{
						BoundingBox.CreateMerged(ref original, ref m_nodes[child2].Aabb, out var result4);
				...
				{
					BoundingBox.CreateMerged(ref original, ref m_nodes[child].Aabb, out var result5);
					BoundingBox.CreateMerged(ref original, ref m_nodes[child2].Aabb, out var result6);
...
>>>>>>> d46cf8619665219cc163a7b28984ced59ed9470d
		...
		public void OverlapAllFrustum<T, Op>(ref BoundingFrustum frustum, ref Op add) where Op : struct, IAddOp<T>
		{
			if (m_root == -1)
<<<<<<< HEAD
			{
...
>>>>>>> d46cf8619665219cc163a7b28984ced59ed9470d
		...
		public void OverlapAllFrustum<T, Op>(ref BoundingFrustum frustum, float tSqr, ref Op add) where Op : struct, IAddOp<T>
		{
			if (m_root == -1)
<<<<<<< HEAD
			{
