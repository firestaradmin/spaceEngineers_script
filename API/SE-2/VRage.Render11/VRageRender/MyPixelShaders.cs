...
{
	internal static class MyPixelShaders
	{
		internal struct Id
		{
			internal int Index;
...
>>>>>>> d46cf8619665219cc163a7b28984ced59ed9470d
		...
		private static readonly MyFreelist<MyShaderInfo<PixelShader>> m_shaders;

		private static readonly Dictionary<MyShaderKey, Id> m_keyToId;

		static MyPixelShaders()
		{
			m_keyToId = new Dictionary<MyShaderKey, Id>();
			m_shaders = new MyFreelist<MyShaderInfo<PixelShader>>(1024);
		}
		...
		internal static Id Create(string file, ShaderMacro[] macros = null)
		{
			...
			myShaderCompilationInfo.Profile = MyShaderProfile.ps_5_0;
			...
			lock (m_keyToId)
			{
				...
				m_shaders.Data[value.Index] = new MyShaderInfo<PixelShader>
				{
					Shader = shader,
					InfoId = infoId,
					File = file
		...
		private static PixelShader GetShader(Id id)
		{
			return m_shaders.Data[id.Index].Shader;
		}

		private static void Init(ref MyShaderCompilationInfo info, out PixelShader shader)
		{
			byte[] shaderBytecode = MyShaderCompiler.Compile(ref info);
			try
			{
				shader = new PixelShader(MyRender11.DeviceInstance, shaderBytecode)
				{
		...
		internal static void OnDeviceEnd()
		{
			lock (m_keyToId)
			{
				foreach (Id value in m_keyToId.Values)
				{
					MyShaderInfo<PixelShader> myShaderInfo = m_shaders.Data[value.Index];
