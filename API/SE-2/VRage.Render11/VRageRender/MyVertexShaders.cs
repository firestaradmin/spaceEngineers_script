...
{
	internal static class MyVertexShaders
	{
		internal struct Id
		{
			internal int Index;
...
>>>>>>> d46cf8619665219cc163a7b28984ced59ed9470d
			public static implicit operator VertexShader(Id id)
			{
				return GetShader(id);
			}
		}

		private static readonly MyFreelist<MyShaderInfo<VertexShader>> m_shaders = new MyFreelist<MyShaderInfo<VertexShader>>(1024);
		...
		internal static Id Create(string file, ShaderMacro[] macros = null)
		{
			...
			myShaderCompilationInfo.Profile = MyShaderProfile.vs_5_0;
			myShaderCompilationInfo.Macros = macros;
			MyShaderCompilationInfo info = myShaderCompilationInfo;
			Init(ref info, out var byteCode, out var shader);
			lock (m_keyToId)
			{
				if (m_keyToId.TryGetValue(key, out value))
				{
				...
				ShaderInfoId infoId = MyShaders.CreateInfo(byteCode, ref info);
				m_shaders.Data[value.Index] = new MyShaderInfo<VertexShader>
				{
					Shader = shader,
					InfoId = infoId,
					File = file
		...
		private static VertexShader GetShader(Id id)
		{
			return m_shaders.Data[id.Index].Shader;
		}

		private static void Init(ref MyShaderCompilationInfo info, out byte[] byteCode, out VertexShader shader)
		{
			byteCode = MyShaderCompiler.Compile(ref info);
			try
			{
				shader = new VertexShader(MyRender11.DeviceInstance, byteCode)
				{
			...
			catch (SharpDXException)
			{
				...
				{
					Init(ref info, out byteCode, out shader);
					return;
				}
		...
		internal static void Recompile()
		{
			lock (m_keyToId)
			{
				...
				foreach (Id value in m_keyToId.Values)
				{
					...
					Init(ref info, out var byteCode, out var shader);
					MyShaders.UpdateByteCode(infoId, byteCode);
					m_shaders.Data[value.Index] = new MyShaderInfo<VertexShader>
					{
						Shader = shader,
						InfoId = infoId,
		...
		internal static void OnDeviceEnd()
		{
			lock (m_keyToId)
			{
				foreach (Id value in m_keyToId.Values)
				{
					MyShaderInfo<VertexShader> myShaderInfo = m_shaders.Data[value.Index];
