...
{
	[PreloadRequired]
	internal class MySpaceWorldGenerator
	{
		public static Dictionary<string, Vector3> m_compatStaticAsteroids;

		static MySpaceWorldGenerator()
		{
			m_compatStaticAsteroids = new Dictionary<string, Vector3>(128);
		...
		private static void MyWorldGenerator_AfterGenerate(ref MyWorldGenerator.Args args)
		{
		...
		{
			float scale = Math.Max(args.Scenario.AsteroidClustersOffset / 700f, 1f);
			...
			while (args.AsteroidAmount > 0 && list.Count > 0)
			{
				int index = MyRandom.Instance.Next(0, list.Count);
				int num = list[index];
				list.RemoveAtFast(index);
				AddPregeneratedCluster(ref args, "asteroid" + num, scale);
			}
		}
		...
		{
			Vector3 vector = m_compatStaticAsteroids[name] * scale;
			MyWorldGenerator.AddAsteroidPrefab("Pregen\\" + name, vector, name);
		}
	}
}
