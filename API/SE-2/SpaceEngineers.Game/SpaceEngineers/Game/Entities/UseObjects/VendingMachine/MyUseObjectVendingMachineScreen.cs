...
namespace SpaceEngineers.Game.Entities.UseObjects.VendingMachine
{
	...
	public class MyUseObjectVendingMachineScreen : MyUseObjectBase
	{
		...
		public MyUseObjectVendingMachineScreen(IMyEntity owner, string dummyName, MyModelDummy dummyData, uint key)
		...
		public override void Use(UseActionEnum actionEnum, IMyEntity userEntity)
		{
			...
			{
			case UseActionEnum.Manipulate:
				if (userRelationToOwner != MyRelationsBetweenPlayerAndBlock.Owner && !MySession.Static.AdminSettings.HasFlag(AdminSettingsEnum.UseTerminals))
				{
					MyHud.Notifications.Add(MyNotificationSingletons.AccessDenied);
				}
				else if (myVendingMachine.IsWorking && m_screen == null)
				{
					MyStoreBlockViewModel myStoreBlockViewModel = new MyStoreBlockViewModel(myVendingMachine);
					myStoreBlockViewModel.IsBuyTabVisible = false;
					myStoreBlockViewModel.IsSellTabVisible = false;
					myStoreBlockViewModel.AdministrationViewModel.IsTabNewOrderVisible = false;
					m_screen = MyGuiSandbox.CreateScreen<MyGuiScreenStoreBlock>(new object[1] { myStoreBlockViewModel });
					m_screen.Closed += Screen_Closed;
					MyGuiSandbox.AddScreen(m_screen);
				}
				break;
		...
		{
			m_screen = null;
			(base.Owner as MyVendingMachine)?.UpdateStoreContent();
		}
	}
}
